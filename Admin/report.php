        ...
        <nav class="sidebar sidebar-offcanvas" id="sidebar">
            <div class="sidebar-brand-wrapper d-none d-lg-flex align-items-center justify-content-center fixed-top">
                <a class="sidebar-brand brand-logo" href="dashboard.php"><img src="../Assets/images/EcoVibe_Logo_FS.png"
                        alt="logo" /></a>
                <a class="sidebar-brand brand-logo-mini" href="dashboard.php"><img
                        src="../Assets/images/EcoVibe_Logo_SS.png " alt="logo" /></a>
            </div>
            <ul class="nav">
                ...
                <li class="nav-item menu-items">
                    <span class="nav-link" data-page="no-page" data-toggle="collapse" href="#m-music"
                        aria-expanded="false" aria-controls="m-music">
                        <span class="menu-icon">
                ...
                <li class="nav-item menu-items">
                    <a class="nav-link" data-page="no-page" data-toggle="collapse" href="#m-playlist"
                        aria-expanded="false" aria-controls="m-playlist">
                        <span class="menu-icon">
                    ...
                    <div class="collapse" id="m-playlist">
                        <ul class="nav flex-column sub-menu">
                            ...
                            <li class="nav-item"> <a class="nav-link" data-page="manage-playlist.php?view=1">View</a>
                            </li>
                        </ul>
                    </div>
                </li>
                    ...
                    <div class="collapse" id="m-users">
                        <ul class="nav flex-column sub-menu">
                            <li class="nav-item"> <a class="nav-link"
                                    data-page="manage-artists.php?artistData=1">Artists</a></li>
        ...
        <div class="container-fluid page-body-wrapper">
                            ...
                            <a class="nav-link" id="profileDropdown" href="#" data-toggle="dropdown">
                                <div class="navbar-profile">
                                    <img class="img-sm rounded-circle"
                                        src="<?= isset($profileImg) ? $profileImg : ''; ?>" alt="logo">
            ...
            <div class="main-panel" id="main-panel">
                ...
                <div class="row pt-3">
                    ...
                    <div class="col-xxl-4 col-md-6 pt-2">
                        <div class="card info-card sales-card">
                            <h3 class="card-title text-center">TOP 10 MOST STREAMED SONGS</h3>
                            <canvas id="myPieChart"></canvas>

                            <?php
                            $usersRef = $database->getReference('users');

                            // INITIALIZE VARIABLES TO STORE INFORMATION ABOUT THE MOST STREAMED SONG
                            $maxStreamCount = 0;
                            $maxStreamedSongTitle = null;
                            $totalStreams = 0;
                            $songPercentages = [];

                            // LOOP THROUGH EACH USER
                            foreach ($usersRef->getValue() as $userId => $userData) {
                                // CHECK IF THE USER HAS SONGS
                                if (isset($userData['songs'])) {
                                    // LOOP THROUGH EACH SONG OF THE USER
                                    foreach ($userData['songs'] as $songId => $songData) {
                                        // INCREMENT TOTAL STREAMS
                                        $totalStreams += $songData['song_stream_count'];
                                    }
                                }
                            }

                            // LOOP THROUGH EACH USER AGAIN TO CALCULATE SONG PERCENTAGES
                            foreach ($usersRef->getValue() as $userId => $userData) {
                                // CHECK IF THE USER HAS SONGS
                                if (isset($userData['songs'])) {
                                    // LOOP THROUGH EACH SONG OF THE USER
                                    foreach ($userData['songs'] as $songId => $songData) {
                                        // CALCULATE THE PERCENTAGE OF STREAMS FOR THE CURRENT SONG
                                        $percentage = ($songData['song_stream_count'] / $totalStreams) * 100;
                                        // STORE THE SONG TITLE AND ITS PERCENTAGE OF STREAMS
                                        $songPercentages[$songData['song_title']] = $percentage;
                                    }
                                }
                            }

                            // SORT THE SONG PERCENTAGES IN DESCENDING ORDER
                            arsort($songPercentages);

                            // SELECT ONLY THE TOP 10 SONGS
                            $top10SongPercentages = array_slice($songPercentages, 0, 10, true);

                            // // Output song percentages for the top 10 songs
                            // foreach ($top10SongPercentages as $songTitle => $percentage) {
                            //   echo '<p style="color: white;">Song: ' . $songTitle . ' - Percentage of Streams: ' . number_format($percentage, 2) . '%</p>';
                            // }
                            ?>
                            ...
                            <script>
                                // Parse PHP data and convert to JSON
                                var top10SongPercentages = <?php echo json_encode($top10SongPercentages); ?>;
                                var chartData = {
                                    labels: Object.keys(top10SongPercentages),
                                    datasets: [{
                                        data: Object.values(top10SongPercentages),
                                        backgroundColor: [
                                            'rgba(255, 99, 132, 0.8)',
                                            'rgba(54, 252, 235, 0.8)',
                                            'rgba(255, 206, 86, 0.8)',
                    ...
                    <div class="col-xxl-4 col-md-12 ">
                        <h5 class="card-title text-center">AGE GROUP USERS</h5>
                        <?php
                        try {
                            // Include your Firebase database connection file
                            ...
                            // Iterate through user data to calculate age and count users in each age group
                            foreach ($userData as $userId => $user) {
                                // Calculate age based on date of birth
                                $dateOfBirth = isset($user['date_of_birth']) ? $user['date_of_birth'] : $user['dateOfBirth'];
                                $birthYear = date('Y', strtotime($dateOfBirth));
                                $age = $currentYear - $birthYear;

                                // Categorize users into different age groups
                                if ($age >= 13 && $age <= 18) {
                                    $childCount++;
                            ...
                            $ageGroups = array("Child", "Young Adult", "Middle Age");
                            $userCounts = array($childCount, $youngAdultCount, $middleAgeCount);
                            foreach ($ageGroups as $index => $ageGroup) {
                                echo "<h4><option value=\"$ageGroup\" style=\"color: white;\">$ageGroup: $userCounts[$index]</option></h4 >";
                            }
                        } catch (Exception $e) {
